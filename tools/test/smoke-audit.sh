...
if [[ "$EVENT_COUNT" -lt 20 ]]; then echo "smoke failed: expected >=20 audit_event, got $EVENT_COUNT" >&2; exit 1; fi
...
for event in BOOT_STARTED HANDOFF_TO_KMAIN CONSOLE_READY AUDIT_BACKEND_READY HW_DISCOVERY_STARTED HW_DISCOVERY_COMPLETED HW_SNAPSHOT_PERSISTED AI_PROPOSAL_CREATED AI_PROPOSAL_SHOWN AI_PROPOSAL_APPROVED AI_PROPOSAL_REJECTED GUIDED_CONSOLE_SHOWN MENU_OPTION_SELECTED EMERGENCY_SESSION_STARTED EMERGENCY_INPUT_RECEIVED STRUCTURED_RESPONSE_SHOWN; do
