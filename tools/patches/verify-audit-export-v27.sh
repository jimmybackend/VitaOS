...
DOC="$ROOT/docs/audit/audit-verification-export.md"

if [[ ! -f "$AUDIT_H" || ! -f "$AUDIT_C" || ! -f "$COMMAND_C" ]]; then
  echo "[verify-v27] ERROR: run from VitaOS repository root" >&2
  exit 1
fi
...
required=(
  ...
  "AUDIT_VERIFY_TEXT_EXPORT_PATH"
  "AUDIT_VERIFY_JSONL_EXPORT_PATH"
  "export audit verify"
)

for sym in "${required[@]}"; do
  if ! grep -Rqs "$sym" "$AUDIT_H" "$AUDIT_C" "$COMMAND_C" "$DOC" 2>/dev/null; then
    echo "[verify-v27] ERROR: missing $sym" >&2
    exit 1
  fi
done
...
if grep -RInE '(^#!.*python|python3|[^A-Za-z0-9_]python[^A-Za-z0-9_])' \
  ...
  echo "[verify-v27] ERROR: unexpected Python reference" >&2
  exit 1
fi

echo "[verify-v27] OK: audit verification export patch present"
