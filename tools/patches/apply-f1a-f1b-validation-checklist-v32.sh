...
# Documentation-only patch. Bash only. No Python.

ROOT=$(pwd)
DOC_DIR="$ROOT/docs/validation"
DOC="$DOC_DIR/f1a-f1b-validation-checklist.md"

if [[ ! -f "$ROOT/Makefile" || ! -d "$ROOT/kernel" || ! -d "$ROOT/include" ]]; then
  echo "[v32] ERROR: run this from the VitaOS repository root" >&2
  exit 1
fi
...
echo "[v32] F1A/F1B validation checklist written: docs/validation/f1a-f1b-validation-checklist.md"
