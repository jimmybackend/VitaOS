...
# VitaOS patch v21: UEFI mouse wheel scroll + visual scroll indicator.
...
if [[ ! -f "$UEFI_ENTRY" || ! -f "$NEON_C" || ! -f "$NEON_H" || ! -f "$MAKEFILE" ]]; then
  echo "[v21] ERROR: run this from a VitaOS repository root containing current UEFI neon files" >&2
  exit 1
fi

mkdir -p "$DOC_DIR"

# Ensure the UEFI neon text renderer is compiled into BOOTX64.EFI.
...
# Header prototypes for wheel status and wheel scroll.
...
# Renderer state: remember whether wheel-compatible Z movement has been observed.
if ! grep -q 'wheel_available' "$NEON_C"; then
  perl -0pi -e 's/(    uint32_t view_offset;\n)/$1    uint32_t wheel_available;\n/' "$NEON_C"
fi

# Draw a small right-side scroll indicator and wheel status marker.
...
# Repaint the indicator on every body render.
if ! grep -q 'draw_scroll_indicator();' "$NEON_C"; then
  perl -0pi -e 's/(    if \(g_neon\.view_offset > 0U\) \{\n)/    draw_scroll_indicator();\n\n$1/' "$NEON_C"
fi

# Public wheel functions.
if ! grep -q 'vita_uefi_neon_text_set_wheel_available' "$NEON_C"; then
  cat >> "$NEON_C" <<'CADD'
...
# UEFI entry needs neon text hooks, PageUp/PageDown, and pointer protocol polling.
...
if ! grep -q 'uefi_pointer_init' "$UEFI_ENTRY"; then
...
if ! grep -q 'vita_uefi_neon_text_write_line' "$UEFI_ENTRY"; then
...
# Basic sanity checks: no Python and expected symbols present.
if grep -RInE '(^#!.*python|python3|[^A-Za-z0-9_]python[^A-Za-z0-9_])' "$MAKEFILE" "$UEFI_ENTRY" "$NEON_C" "$NEON_H" 2>/dev/null; then
  echo "[v21] ERROR: unexpected Python reference in patched runtime/build files" >&2
  exit 1
fi

for sym in \
  'vita_uefi_neon_text_scroll_wheel' \
  'draw_scroll_indicator' \
  'uefi_poll_pointer_scroll' \
  'g_simple_pointer_protocol_guid' \
  'g_absolute_pointer_protocol_guid'; do
  if ! grep -q "$sym" "$UEFI_ENTRY" "$NEON_C" "$NEON_H" 2>/dev/null; then
    echo "[v21] ERROR: expected symbol missing after patch: $sym" >&2
    exit 1
  fi
done

echo "[v21] mouse wheel scroll and visual scroll indicator patch applied"
